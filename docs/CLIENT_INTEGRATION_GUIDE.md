...
giving H/H, L/H, H/L, and L/L compositions. `leaf_mode` is
`"bounded_continuation"` (default, preserving classic ISMCTS behavior) or
...
Artifact references must not contain control characters. Invalid reference syntax
is rejected during request validation. If a syntactically valid artifact cannot
be loaded (including a missing optional Torch installation or an unsafe resolved
path), the server logs a warning and uses heuristic components instead. The
persisted specification retains the requested sources and artifact identity.
Runtime loading happens off the event loop and outside the game lock.
