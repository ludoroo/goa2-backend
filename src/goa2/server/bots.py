...
class _ApplyDecisionOutcome:
    """Distinguish a harmless stale snapshot from a genuine apply failure."""

    result: SessionResult | None = None
    is_stale: bool = False

    @classmethod
    def success(cls, result: SessionResult) -> _ApplyDecisionOutcome:
        return cls(result=result)

    @classmethod
    def stale(cls) -> _ApplyDecisionOutcome:
        return cls(is_stale=True)

    @classmethod
    def failed(cls) -> _ApplyDecisionOutcome:
        return cls()
...
async def _bot_drive_worker(game: ManagedGame, registry: GameRegistry) -> None:
    ...
    while iterations < max_iterations:
        ...
        # ------------------------------------------------------------------ #
        outcome = await _apply_bot_decision(game, registry, decision, agents)
        if outcome.is_stale:
            # Another mutation won while this decision was computing. The
            # current worker is still the game's only scheduled wake-up, so
            # yield once and resnapshot instead of exiting and stranding bot
            # work that the winning mutation already tried to schedule.
            await asyncio.sleep(0)
            continue
        applied = outcome.result
        if applied is None:
            # Genuine apply/side-effect failures still halt. Retrying those
            # here would tight-loop a broken agent or engine path.
            return
...
async def _apply_bot_decision(
    ...
    decision: BotDecision,
    agents: dict[str, Agent],
) -> _ApplyDecisionOutcome:
    """Apply one :class:`BotDecision` through the standard locked mutation.

    Returns a typed outcome so the worker can resnapshot a stale decision while
    still halting on engine, logging, persistence, or broadcast failures. On
    every failure path after clock acceptance, ``finalize_timed_mutation`` is
    guaranteed to run in ``finally``, matching REST/WS mutations.
    """
    result: SessionResult | None = None
        ...
        async with game.outbound_lock:
            async with game.lock:
                ...
                if not _is_decision_still_valid(live_state, game.last_result, decision, agents):
                    ...
                    return _ApplyDecisionOutcome.stale()
    ...
    if result is None:
        return _ApplyDecisionOutcome.failed()
    return _ApplyDecisionOutcome.success(result)
...
def auto_ready_bot_heroes(game: ManagedGame, at_ms: int | None = None) -> bool:
    ...
    Timed matches require every hero to ready when waiting to start, resuming
    from inactivity, or unpausing. A game with humans + bots would otherwise
    stall because no client submits a fresh ready flag on behalf of a bot.
    This helper is idempotent (a hero already listed in ``ready_hero_ids`` is
