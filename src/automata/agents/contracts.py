...
Planning uses :class:`PlanningDecision`, which distinguishes
  ...
  intent explicitly rather than overloading ``None``.
"""
...
class Agent(Protocol):
    ...
    def choose_planning(self, state: GameState, hero: Hero) -> PlanningDecision:
        """Return the next explicit planning action for ``hero``."""
        ...
    ...
    def choose_input(
        ...
        *,
        ...
        - Search-backed agents (:class:`~automata.agents.ISMCTSAgent`)
          require it non-empty to anchor the root and reject wrong-team /
          non-owned calls at the public boundary.
        - Simpler agents (Random / Heuristic) ignore it because they read only
          ``state`` and ``request``.
...
# Planning decisions
#
# ``PlanningDecision`` lets the driver call
...
class PlanningKind(StrEnum):
    ...
    A ``StrEnum`` so the values serialize cleanly as ``"COMMIT"``, ``"FINISH"``,
    and ``"PASS"``.
