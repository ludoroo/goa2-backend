...
from automata.search.scheduling import RootSearchPlan, observe_root_search_plans
...
class TelemetryEvent(BaseModel):
    ...
    effective_leaf_mode: str | None = None
    semantic_role: str | None = None
    requested_iterations: int | None = Field(default=None, ge=0)
    effective_iterations: int | None = Field(default=None, ge=0)
    root_coverage_target: int | None = Field(default=None, ge=0)
...
class _DecisionPlanHolder:
    """One-decision plan handoff that is explicitly cleared after telemetry."""

    plan: RootSearchPlan | None = None

    def record(self, plan: RootSearchPlan) -> None:
        self.plan = plan
...
class _RecordingStrategy:
    ...
    def select(
        ...
        started = self._clock()
        plan_holder = _DecisionPlanHolder()
        self._emit_decision(
            "decision_started",
            state,
            perspective_team,
        ...
        try:
            try:
                with (
                    observe_root_search_plans(plan_holder.record),
                    _decision_timeout(self._decision_timeout_seconds),
                ):
                ...
                if plan_holder.plan is not None:
                    exc.attach_root_search_plan(plan_holder.plan)
                self._emit_decision(
                    "decision_failed",
                    state,
                    perspective_team,
                    ...
                    max(0.0, self._clock() - started),
                    result,
                    exc.root_search_plan or plan_holder.plan,
...
class SelfPlayWorker:
    ...
    def _emit_decision(
        ...
        *,
        ...
        request_type = (
            root_plan.request_type
            if root_plan is not None
            else request.request_type.value if request is not None else None
        )
        completed_visits: int | None = None
        visited_legal_count: int | None = None
        legal_coverage: float | None = None
        statistics = result.search_result if result is not None else None
        if statistics is not None:
            children = statistics.root.children
        ...
        self.telemetry(
            TelemetryEvent(
                ...
                schedule_id=(
                    root_plan.schedule_id
                    if root_plan is not None
                    else statistics.schedule_id if statistics is not None else None
                ),
                effective_leaf_mode=(
                    root_plan.effective_leaf_mode.value
                    if root_plan is not None
                    else (
                        statistics.effective_leaf_mode.value
                        if statistics is not None and statistics.effective_leaf_mode is not None
                        else None
                    )
                ),
                semantic_role=(
                    root_plan.semantic_role.value
                    if root_plan is not None
                    else (
                        statistics.semantic_role.value
                        if statistics is not None and statistics.semantic_role is not None
                        else None
                    )
                ),
                requested_iterations=(
                    root_plan.requested_iterations
                    if root_plan is not None
                    else statistics.requested_iterations if statistics is not None else None
                ),
                effective_iterations=(
                    root_plan.effective_iterations
                    if root_plan is not None
                    else statistics.effective_iterations if statistics is not None else None
                ),
                root_coverage_target=(
                    root_plan.root_coverage_target
                    if root_plan is not None
                    else statistics.root_coverage_target if statistics is not None else None
                ),
