...
_INPUT_REQUEST_BUCKETS = ("null", *sorted(_KNOWN_INPUT_REQUEST_TYPES))
_SEMANTIC_ROLE_BUCKETS = tuple(sorted(item.value for item in DecisionSemanticRole))
_CAN_SKIP_BUCKETS = ("false", "true")
...
class PolicyMetricInput:
    ...
    input_request_type: str | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.input_request_type is not None
            and self.input_request_type not in _KNOWN_INPUT_REQUEST_TYPES
        ):
            raise ValueError("input request type must be known or null for CARD planning")
        if self.semantic_role is not None and not isinstance(
            self.semantic_role, DecisionSemanticRole
        ):
            raise ValueError("semantic role must be a DecisionSemanticRole")
        if self.can_skip is not None and type(self.can_skip) is not bool:
            raise ValueError("can_skip must be boolean")
        if self.semantic_role is None:
            if self.input_request_type is not None or self.can_skip is not None:
                raise ValueError("typed decision metric context must be supplied together")
        elif self.can_skip is None:
            raise ValueError("typed decision metric context must include can_skip")
        elif self.semantic_role is DecisionSemanticRole.PLANNING:
...
def _input_request_bucket(row: PolicyMetricInput) -> str | None:
    if row.input_request_type is not None:
        return row.input_request_type
    if row.semantic_role is DecisionSemanticRole.PLANNING:
        return "null"
    return None
...
def policy_metrics(examples: Iterable[PolicyMetricInput], *, top_k: int = 3) -> dict[str, Any]:
    ...
    result["by_input_request_type"] = _policy_buckets(
        rows,
        measured,
        top_k,
        _input_request_bucket,
        expected_buckets=_INPUT_REQUEST_BUCKETS,
    )
    result["by_semantic_role"] = _policy_buckets(
        rows,
        measured,
        top_k,
        lambda row: row.semantic_role.value if row.semantic_role is not None else None,
        expected_buckets=_SEMANTIC_ROLE_BUCKETS,
    )
    result["by_can_skip"] = _policy_buckets(
        rows,
        measured,
        top_k,
        lambda row: str(row.can_skip).lower() if row.can_skip is not None else None,
        expected_buckets=_CAN_SKIP_BUCKETS,
    )
    return result
...
def _measure_policy(row: PolicyMetricInput, top_k: int) -> _PolicyDecision:
    ...
    pairwise = sum(comparisons) / len(comparisons) if comparisons else None
...
def _aggregate_policy(
    ...
    informative_rows = [row for row in rows if measured[id(row)].pairwise_accuracy is not None]
    return {
        "count": len(rows),
        "game_count": len({row.game_id for row in rows}),
        "multi_candidate_count": sum(len(row.target_probabilities) > 1 for row in rows),
        "pairwise_count": len(informative_rows),
        "pairwise_game_count": len({row.game_id for row in informative_rows}),
...
def _policy_buckets(
    ...
    *,
    ...
    for row in rows:
        ...
        if bucket is not None:
            grouped[bucket].append(row)
    buckets = tuple(expected_buckets) or tuple(sorted(grouped))
    return {bucket: _aggregate_policy(grouped[bucket], measured, top_k) for bucket in buckets}
...
class _PolicyAggregate:
    ...
    def add(self, game_id: str, decision: _PolicyDecision, *, candidate_count: int) -> None:
        self.count += 1
        self.multi_candidate_count += int(candidate_count > 1)
    ...
    def compute(self, top_k: int) -> dict[str, Any]:
        ...
        return {
            ...
            "pairwise_count": sum(game.pairwise_accuracy.count for game in self.games.values()),
            "pairwise_game_count": sum(
                game.pairwise_accuracy.count > 0 for game in self.games.values()
            ),
...
class _PolicyMetricsAccumulator:
    def __init__(self, top_k: int) -> None:
        ...
        self.context_buckets: dict[str, dict[str, _PolicyAggregate]] = {
    ...
    @staticmethod
    def _context_values(row: PolicyMetricInput) -> tuple[tuple[str, str | None], ...]:
        return (
            (
                "by_input_request_type",
                _input_request_bucket(row),
            ),
            (
                "by_semantic_role",
                row.semantic_role.value if row.semantic_role is not None else None,
            ),
            (
                "by_can_skip",
                str(row.can_skip).lower() if row.can_skip is not None else None,
            ),
        )

    def add(self, row: PolicyMetricInput) -> None:
        decision = _measure_policy(row, self.top_k)
        self.overall.add(row.game_id, decision, candidate_count=len(row.target_probabilities))
        for output_name, attribute in _METRIC_DIMENSIONS:
            ...
            if bucket is not None:
                ...
                aggregate.add(row.game_id, decision, candidate_count=len(row.target_probabilities))
        for output_name, bucket in self._context_values(row):
            if bucket is None:
                continue
            groups = self.context_buckets[output_name]
            aggregate = groups.get(bucket)
            if aggregate is None:
                aggregate = groups[bucket] = _PolicyAggregate()
            aggregate.add(row.game_id, decision, candidate_count=len(row.target_probabilities))
    ...
    def compute(self) -> dict[str, Any]:
        ...
        for output_name, expected in (
            ("by_input_request_type", _INPUT_REQUEST_BUCKETS),
            ("by_semantic_role", _SEMANTIC_ROLE_BUCKETS),
            ("by_can_skip", _CAN_SKIP_BUCKETS),
        ):
            groups = self.context_buckets[output_name]
            result[output_name] = {
                bucket: groups.get(bucket, _PolicyAggregate()).compute(self.top_k)
