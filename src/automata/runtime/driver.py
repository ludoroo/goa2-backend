...
Provides reusable decision-ownership and application helpers:
...
bot coordinator uses exactly this seam.
...
- **No arbitrary fallback.** If a decision is addressed to a hero/team with
  no mapped bot, we return ``None``. Falling through to any agent would be
  unsafe once humans and bots share a game.
- **Illegal bot output raises.** An invalid planning intent is a bug in the
  agent — the driver raises
...
- **Emmitt's second commit.** After a two-card-capable bot commits its first
  card, the driver re-consults ``choose_planning`` for an explicit ``FINISH``
  or second ``COMMIT`` decision.
...
from ..agents.contracts import Agent, PlanningDecision, PlanningKind
...
class IllegalBotDecisionError(ValueError):
    ...
    - ``choose_planning`` commits a card that is not in the hero's hand,
    - it passes while cards remain, or
    - it finishes outside an open two-card window.
    ...
    bot without inspecting driver internals.
    """
...
def _call_choose_input(
    ...
    ``owned_hero_ids``. We pass it unconditionally. If an implementation drops
    the kwarg it will
...
def inspect_next_owner(
    ...
    **Cheap** (no policy invocation): this walks the same ordering
    ...
    but does **not** call ``agent.choose_planning`` / ``agent.choose_input``. The
...
def apply_decision(session: GameSession, decision: BotDecision) -> SessionResult:
    ...
    if decision.kind is DecisionKind.PLANNING:
        ...
        if plan.kind is PlanningKind.COMMIT:
            ...
            live_card = (
                next((card for card in hero.hand if card.id == plan.card.id), None)
                if hero is not None
                else None
            )
...
def _planning_owner(state: GameState, agents: Mapping[str, Agent]) -> str | None:
    ...
    ``choose_planning`` call, no hand/plan validation. Because we do not
...
def _inspect_planning(state: GameState, agents: Mapping[str, Agent]) -> BotDecision | None:
    ...
    for hero_id in _all_hero_ids(state):
        ...
        if planning_open_for_second_card(state, HeroID(hero_id)):
            planning = agent.choose_planning(state, hero)
            if planning.kind is PlanningKind.PASS:
                raise IllegalBotDecisionError(hero_id, "cannot pass during second-card planning")
            if planning.kind is PlanningKind.COMMIT and planning.card not in hero.hand:
                assert planning.card is not None
                raise IllegalBotDecisionError(
                    hero_id,
                    f"choose_planning returned card {planning.card.id!r} not in hand "
        ...
        if planning.kind is PlanningKind.FINISH:
            raise IllegalBotDecisionError(hero_id, "cannot finish before first card")
        if planning.kind is PlanningKind.PASS and hero.hand:
            raise IllegalBotDecisionError(hero_id, "cannot pass with cards in hand")
        if planning.kind is PlanningKind.COMMIT and planning.card not in hero.hand:
            assert planning.card is not None
            raise IllegalBotDecisionError(
                hero_id,
