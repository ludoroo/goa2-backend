...
class SearchConfig:
    ...
    # Cooperative wall-clock bound enforced inside search. Server-created
    # agents always set this; ``None`` keeps offline/evaluation callers on an
    # iteration-only budget.
    decision_timeout_seconds: float | None = None

    # Deterministic guard against an engine/default-policy loop inside one
    # determinized simulation.
    max_advance_steps: int = 10_000
    ...
    # Versioned root scheduling derived from the validated request semantics.
    # ``None`` preserves the complete legacy schedule (including the separate
    # adaptive HEX option above); version 1 applies the frozen request-aware
    # iteration, coverage, and effective-horizon plan. Version 2 retains those
    # budgets while using STABLE_TURN for eligible resolution inputs.
    request_schedule_version: int | None = None
    ...
    def __post_init__(self) -> None:
        for field_name in (
            ...
            schedule_version = getattr(self, field_name)
            if schedule_version is not None and (
                isinstance(schedule_version, bool)
                or not isinstance(schedule_version, int)
                or schedule_version
                not in ({1} if field_name == "adaptive_hex_root_schedule_version" else {1, 2})
            ):
                allowed = "1" if field_name == "adaptive_hex_root_schedule_version" else "1 or 2"
                raise ValueError(f"{field_name} must be None or integer {allowed}")
        for field_name in (
            "max_advance_steps",
        ...
        if self.decision_timeout_seconds is not None and (
            ...
            or not isinstance(self.decision_timeout_seconds, (int, float))
            or not math.isfinite(self.decision_timeout_seconds)
            or self.decision_timeout_seconds <= 0.0
        ):
            raise ValueError("decision_timeout_seconds must be None or a finite positive number")
...
def parse_learned_lh_search_config(
    ...
    exactly the same complete validation boundary. Cooperative serving deadlines
    are forbidden here: offline runs must complete the visit budget or fail under
    their outer, game-censoring watchdog rather than publish partial search.
    """
    if "seed" in raw:
    ...
    for field in ("max_advance_steps", "max_advance_transitions", "max_forced_decisions"):
        resolved[field] = _strict_int(resolved, field, minimum=1)
    if resolved["decision_timeout_seconds"] is not None:
        raise ValueError("offline learned search requires decision_timeout_seconds=None")
    ...
    for field in (
        ...
        schedule = resolved[field]
        allowed_versions = {1} if field == "adaptive_hex_root_schedule_version" else {1, 2}
        if schedule is not None and (
            isinstance(schedule, bool)
            or not isinstance(schedule, int)
            or schedule not in allowed_versions
        ):
            allowed_text = "1" if field == "adaptive_hex_root_schedule_version" else "1 or 2"
            raise ValueError(
                f"search-config field {field!r} must be null or integer {allowed_text}"
            )
