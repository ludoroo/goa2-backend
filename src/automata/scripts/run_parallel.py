...
from automata.scripts.generate_joint_bootstrap import CheckpointRow, _read_checkpoint
from automata.training.dataset import JointDatasetRow, load_joint_dataset, write_joint_dataset
...
class ProgressReporter:
    """Parent-process progress renderer with deterministic stream and clock seams."""
    ...
    def __init__(
        ...
        *,
        ...
        self._last_completed = initial_completed
        self._rate = 0.0
        self._closed = False
        self._last_counts: ProgressCounts | None = None
        self._last_active_workers: int | None = None

    def report(self, counts: ProgressCounts, *, active_workers: int) -> None:
        now = self.clock()
        elapsed = now - self._last_time
        advanced = counts.completed - self._last_completed
        if elapsed > 0 and advanced > 0:
            self._rate = advanced / elapsed
        ...
        percent = 100.0 * counts.completed / self.total if self.total else 100.0
        width = 20
        filled = min(width, int(width * counts.completed / self.total)) if self.total else width
        text = (
            f"[{'#' * filled}{'-' * (width - filled)}] "
            f"{counts.completed}/{self.total} {percent:.1f}% "
            f"workers {active_workers}/{self.workers} {self._rate:.2f} games/s "
            f"ETA {eta_text} ok {counts.success} failed {counts.failed} "
            f"timeout {counts.timed_out}"
        )
        if self.interactive:
            self.stream.write(f"\r\033[2K{text}")
            self.stream.flush()
        else:
            print(f"joint-bootstrap progress {text}", file=self.stream, flush=True)
        self._last_time = now
        self._last_completed = counts.completed
        self._last_counts = counts
        self._last_active_workers = active_workers

    def close(self, counts: ProgressCounts, *, active_workers: int) -> None:
        if self._closed:
            return
        if counts != self._last_counts or active_workers != self._last_active_workers:
            self.report(counts, active_workers=active_workers)
        if self.interactive:
            self.stream.write("\n")
            self.stream.flush()
        self._closed = True
...
def merge_joint_bootstrap(
    ...
    *,
    ...
    """Validate and atomically merge complete joint games in deterministic order.

    Merge uses O(total decision rows) memory at current bootstrap scale; switch to
    a k-way streaming merge before substantially larger runs.
    """
    ...
    for part in parts:
        if not part.exists():
            raise ValueError(f"missing shard output: {part}")
        dataset = load_joint_dataset(part)
        for row in dataset.rows:
            backing = (row.game_id, row.generator_config_id, row.world_seed)
            if backing not in completed:
                raise ValueError(f"orphan decision {row.decision_id!r} has no completed checkpoint")
    ...
    actual_seeds = {row.world_seed for row in rows}
    ...
    for game_id, decision_indexes in indexes.items():
        if decision_indexes != list(range(len(decision_indexes))):
            raise ValueError(f"game {game_id!r} decision indexes are not contiguous from zero")

    write_joint_dataset(output, rows)
    return len(rows)
...
def checkpoint_progress(
    paths: Sequence[Path], *, expected_seeds: set[int] | None = None
) -> ProgressCounts:
    ...
    for path in paths:
        try:
            ...
            continue
        for row in rows:
            if expected_seeds is None or row.world_seed in expected_seeds:
...
def _run_joint_workers(
    ...
    *,
    ...
    initial_counts = (
        checkpoint_progress(checkpoints, expected_seeds=expected_seeds)
        if show_progress
        else ProgressCounts()
    )
    ...
    reporter = (
        ProgressReporter(
            ...
            clock=clock,
            initial_completed=initial_counts.completed,
        )
        if show_progress
        else None
    )
    try:
        ...
        if reporter is not None:
            reporter.report(initial_counts, active_workers=len(unfinished))
        ...
        while unfinished:
            for index in tuple(unfinished):
                ...
                if return_code != 0:
                    failures.append(f"shard {shard.index} exited {return_code}; inspect {log_path}")
            now = clock()
            if reporter is not None and (now >= next_report or not unfinished):
                latest_counts = checkpoint_progress(checkpoints, expected_seeds=expected_seeds)
                reporter.report(latest_counts, active_workers=len(unfinished))
        ...
        if reporter is not None:
            latest_counts = checkpoint_progress(checkpoints, expected_seeds=expected_seeds)
            reporter.close(latest_counts, active_workers=0)
...
def _parser() -> argparse.ArgumentParser:
    ...
    joint.add_argument(
        ...
        metavar="SECONDS",
        help=f"plain-text/TTY update interval (default: {DEFAULT_PROGRESS_INTERVAL:g})",
    )
