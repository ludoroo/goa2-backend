...
from automata.harness.trajectory import NullRecorder, TrajectoryRecorder
from automata.harness.value_boundaries import StableBoundaryObserver, StableBoundaryTracker
...
class DecisionObserver(Protocol):
    """Optional synchronous observer of exact pre-application decisions."""
...
def _validate_agent_coverage(state: GameState, agents: Mapping[str, Agent]) -> None:
    ...
    if missing:
        raise ValueError(f"run_game requires an agent for every hero; missing {sorted(missing)!r}")
...
def run_game(
    ...
    *,
    ...
    boundary_observer: StableBoundaryObserver | None = None,
    ...
    emitted per decision, and the final outcome at game end. This is the
    diagnostic trajectory data. ``boundary_observer`` separately observes
    completed transitions for candidate-free value examples; only a normal
    terminal outcome permits labeling them. Recording is off by default.
    """
...
def continue_game(
    ...
    *,
    ...
    boundaries = (
        StableBoundaryTracker(state, boundary_observer) if boundary_observer is not None else None
    )
    ...
    while steps < max_steps:
        ...
        if decision is None:
            ...
            if boundaries is None:
                ...
                last_result = session.advance(stop_before_step=boundaries.visit_step)
                boundaries.observe(state)
        ...
        if decision_observer is not None:
            decision_observer.record_decision(state, decision)
        ...
        if boundaries is None:
            ...
            boundaries.record_viewer(str(decision.hero_id))
            last_result = apply_decision(session, decision, stop_before_step=boundaries.visit_step)
            boundaries.observe(state)
        _stabilize_runtime_ids(state, last_result, steps)
        report_progress()
