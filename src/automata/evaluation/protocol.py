"""Deterministic resumable evaluation protocol.

Public boundary:

- :class:`AgentSpec`, :class:`GameCase`, :class:`EvaluationGameResult`,
...
# Reason string written to a synthetic ``EvaluationGameResult`` when a case is
...
class EvaluationGameResult:
    ...
    @classmethod
    def from_mapping(cls, row: dict[str, Any]) -> EvaluationGameResult:
...
class EvaluationSummary:
    """Aggregated view over a set of :class:`EvaluationGameResult` rows."""
...
def _atomic_rewrite(path: Path, observations: Iterable[EvaluationGameResult]) -> None:
...
def _child_entrypoint(
    runner: Callable[[GameCase], EvaluationGameResult],
    case: GameCase,
    ...
    - ``("progress", (rounds, steps))`` while a supporting runner is active,
    - ``("ok", EvaluationGameResult)`` on success,
    ...
    """
    try:
        set_progress_callback = getattr(runner, "set_progress_callback", None)
        if callable(set_progress_callback):
            set_progress_callback(
                lambda rounds, steps: child_conn.send(("progress", (rounds, steps)))
            )
        obs = runner(case)
...
def _run_case_with_timeout(
    ...
    Returns an :class:`EvaluationGameResult` on success. On timeout returns a
    synthetic timeout observation with ``winner_side=None`` and
    ``reason=WALL_CLOCK_TIMEOUT_REASON`` after ensuring the child is gone.
    Runners that support progress reporting preserve their latest round and
    step counts; other runners fall back to zero. On child exception raises
    :class:`_ChildFailure`.
        ...
        while True:
            ...
            if parent_conn.poll(remaining):
                ...
                if message[0] == "progress":
                    latest_rounds, latest_steps = message[1]
                    message = None
                    continue
                break
...
def _require_picklable_runner(runner: Callable[[GameCase], EvaluationGameResult]) -> None:
...
def run_protocol(
    ...
    *,
    ...
    - For each remaining case, invokes ``run_case``, validates that the
      returned :class:`EvaluationGameResult` matches the scheduled ``case_id``,
    ...
    try:
        ...
        cached: dict[str, EvaluationGameResult] = {}
        ...
        with checkpoint_path.open("a", encoding="utf-8") as fh:
            for case in cases:
                if timeout_seconds is not None:
                    ...
                    obs = run_case(case)
                if not isinstance(obs, EvaluationGameResult):
                    raise ValueError("run_case must return an EvaluationGameResult")
                if (
...
def load_observations(path: Path) -> list[EvaluationGameResult]:
    ...
    observations: list[EvaluationGameResult] = []
    for idx, line in enumerate(lines):
        ...
        observations.append(EvaluationGameResult.from_mapping(row))
    return observations


def _is_max_steps(obs: EvaluationGameResult) -> bool:
    return obs.reason == "max_steps"


def _is_wall_clock_timeout(obs: EvaluationGameResult) -> bool:
