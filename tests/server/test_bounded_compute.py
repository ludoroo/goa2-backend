...
from automata.search.ismcts.engine import SearchDeadlineExceeded
...
def test_internal_deadline_is_classified_as_search_timeout_without_traceback(
    caplog: pytest.LogCaptureFixture,
) -> None:
    bounded_compute.reset_bounded_compute_metrics()
    fallback_decision = cast(BotDecision, object())
    ...
    def inspect(*_args: object) -> BotDecision:
        ...
        calls += 1
        if calls == 1:
            raise SearchDeadlineExceeded("internal deadline")
        return fallback_decision
    ...
    metrics = bounded_compute.bounded_compute_metrics
    assert result is fallback_decision
    assert metrics.fallback_search_timeout == 1
    assert metrics.fallback_error == 0
    assert any(
        "fallback=search_timeout" in record.getMessage()
        and "source=internal_deadline" in record.getMessage()
