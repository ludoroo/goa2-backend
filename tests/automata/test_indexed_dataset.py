...
def test_tensorized_training_chunks_roundtrip_with_targets_offsets_and_metrics(
    ...
    assert chunks[0].metric_metadata[0].input_request_type == "SELECT_OPTION"
    assert chunks[0].metric_metadata[0].semantic_role is DecisionSemanticRole.OPTION_SELECTION
    assert chunks[0].metric_metadata[0].can_skip is False
    assert {item.name for item in fields(chunks[0].metric_metadata[0])} == {
        "candidate_count",
        "candidate_family",
...
def test_card_planning_metric_metadata_preserves_null_request_type(tmp_path: Path) -> None:
    source = tmp_path / "joint.jsonl"
    row = _row(7, 0)
    card_observation = DecisionObservation(
        schema_version=4,
    ...
    write_joint_dataset(source, (row.model_copy(update={"observation": card_observation}),))

    indexed = open_indexed_dataset(source, tmp_path / "index")
    metadata = next(indexed.iter_game_training_chunks("game-7")).metric_metadata[0]

    assert metadata.input_request_type is None
    assert metadata.semantic_role is DecisionSemanticRole.PLANNING
    assert metadata.can_skip is False
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("metric_context", "CARD planning metric context"),
...
def test_digest_valid_but_invalid_training_payload_is_rejected(
    ...
    elif corruption == "metric_context":
        payload["metric_metadata"][0]["semantic_role"] = "PLANNING"
    else:
...
def test_index_with_old_metric_chunk_format_is_rejected_and_rebuilt(tmp_path: Path) -> None:
    source = tmp_path / "joint.jsonl"
    cache = tmp_path / "index"
    write_joint_dataset(source, (_row(7, 0),))
