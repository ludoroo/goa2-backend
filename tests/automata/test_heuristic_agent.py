...
def test_position_score_uses_the_destination_zone_lane_on_multi_lane_maps() -> None:
    state = GameSetup.create_game(
        map_path="src/goa2/data/maps/across_the_river.json",
        red_heroes=["Razzle"],
        blue_heroes=["Arien"],
        game_type="QUICK",
        seed=31,
    ...
    lane_id = "lane_2"
    zone_id = state.board.lanes[lane_id][3]
    destination = next(iter(state.board.zones[zone_id].hexes))
    for unit in (*state.teams[TeamColor.BLUE].heroes, *state.teams[TeamColor.BLUE].minions):
        state.remove_entity(str(unit.id))

    score = HeuristicAgent()._position_score(state, destination, TeamColor.RED)

    assert state.lane_of_zone(zone_id) == lane_id
    assert score == 10.0 * zones_between(state, TeamColor.RED, lane_id, zone_id)
