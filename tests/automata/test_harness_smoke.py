...
def test_decision_observer_is_opt_in_and_sees_predecision_state() -> None:
    seen: list[tuple[GamePhase, BotDecision]] = []

    class Observer:
        def record_decision(self, state: GameState, decision: BotDecision) -> None:
            seen.append((state.phase, decision))

        def record_outcome(self, **kwargs: Any) -> None:
            pass

    heroes = ["hero_wasp", "hero_xargatha", "hero_arien", "hero_brogan"]
    agents = {hero: RandomAgent(seed=19) for hero in heroes}
    result = run_game(
        ["Wasp", "Xargatha"],
        ["Arien", "Brogan"],
        agents,
...
class _StubAgent:
    ...
    def choose_planning(self, state: GameState, hero: Hero) -> PlanningDecision:
        self.calls.append(("choose_planning", hero.id))
        card = self._card_for.get(hero.id, hero.hand[0] if hero.hand else None)
        if card is not None:
...
def test_emmitt_second_card_choose_none_maps_to_finish() -> None:
    ...
    ``choose_planning`` produces a :class:`PlanningKind.FINISH`, not another
    ...
    def choose(state: GameState, hero: Hero) -> PlanningDecision:
        call_count["n"] += 1
        return PlanningDecision.finish()

    bot: Any = _StubAgent()
    bot.choose_planning = choose  # type: ignore[assignment]
...
def test_emmitt_second_card_choose_planning_maps_to_second_commit() -> None:
...
def test_real_ismcts_can_finish_emmitt_second_card_window(monkeypatch) -> None:
    ...
    monkeypatch.setattr("automata.agents.ismcts_agent.search", choose_finish)
    agent = ISMCTSAgent(SearchConfig(iterations=1, cutoff_limit=1))
...
def test_planning_raises_illegal_decision_for_card_not_in_hand() -> None:
    """A bot planning a foreign :class:`Card` is a bug —
...
def test_planning_raises_when_bot_declines_with_nonempty_hand() -> None:
    ...
    PASS while it still has cards (and has no open Emmitt window), the driver
    raises :class:`IllegalBotDecisionError`."""
    state = _new_game(["Wasp"], ["Arien"])
    wasp = state.teams[TeamColor.RED].heroes[0]
    ...
    with pytest.raises(IllegalBotDecisionError) as excinfo:
        inspect_next_decision(state, {wasp.id: bot}, last_result=None)
    assert excinfo.value.hero_id == wasp.id
    assert "cannot pass" in excinfo.value.reason
...
def test_emmitt_finish_is_legal_none_and_does_not_raise() -> None:
    ...
    two-card window. ``choose_planning`` returning FINISH after the first
    commit is valid, not a bug."""
    state = _emmitt_state()
    session = GameSession(state)
    emmitt = state.teams[TeamColor.RED].heroes[0]
...
def test_driver_raises_typeerror_if_agent_drops_owned_hero_ids_kwarg() -> None:
    ...
    class _BrokenAgent:
        def choose_planning(self, state: GameState, hero: Hero) -> PlanningDecision:
            return PlanningDecision.commit(hero.hand[0]) if hero.hand else PlanningDecision.pass_()
...
def test_driver_rejects_illegal_input_selection_planning_path_regressed() -> None:
    ...
    class _BadPlanner:
        def choose_planning(self, state, hero):
            ...
            return PlanningDecision.commit(fake)
