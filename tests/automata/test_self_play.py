...
def test_default_agent_seed_and_identity_preserve_the_legacy_derivation() -> None:
    module = _module()
    config = _config()

    assert (
        config.generator_config_id
        == "df158c6f9798f0409a32c1ab5b88771461f54b77d21d90ca9d1994ebe732a842"
    )
    assert module.agent_seed(config, 20_000, "RED") == 5_050_198_945_769_820_610
    assert module.agent_seed(config, 20_000, "BLUE") == 13_162_249_952_427_760_353
...
def test_decision_timeout_discards_game_and_continues_with_actionable_telemetry(
    ...
    spec = module.WorkerSpec(
        worker_id=0,
        config=_config(decision_timeout_seconds=0.02),
        games=_games(20_000, 20_001),
    )
    events: list[Any] = []

    class SlowSecondStrategy(_ImprovedStrategy):
        def __init__(self) -> None:
            self.calls = 0
    ...
    def strategy_factory(_runtime: object, game: Any, _side: str, _seed: int) -> Any:
        return SlowSecondStrategy() if game.world_seed == 20_000 else _ImprovedStrategy()
    ...
    assert all(event.completed_visits == 4 for event in successful)
...
def test_search_progression_failure_discards_game_and_continues_with_diagnostics(
    ...
    def strategy_factory(_runtime: object, game: Any, _side: str, _seed: int) -> Any:
        if game.world_seed == first_game.world_seed:
            return FailSecondDecisionStrategy()
...
def test_self_play_artifact_validation_requires_every_scheduled_mode_and_hero(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    def capturing_load(_cls: Any, _path: Path, *, requirements: Any) -> Any:
        ...
        return SimpleNamespace(
            schema=SimpleNamespace(observation_schema_version=3),
