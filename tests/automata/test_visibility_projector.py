...
def test_projected_topology_matches_neutral_reference_without_unit_immunity() -> None:
    state = _snapshot_state()
    state.active_effects.extend(
        [
            ActiveEffect(
                id="split",
    ...
    positioned = sorted(neutral.entity_locations.items(), key=lambda item: str(item[0]))
    expected = {
        str(source_id): {
            str(target_id): {
    ...
    assert snapshot.public_state["automata_topology"]["unit_relationships"] == expected
    assert expected["hero_razzle_piece_1"]["hero_ally"]["is_straight_line"] is False
