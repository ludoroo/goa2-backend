...
def test_success_writes_canonical_manifest_and_immutable_inference_artifact(tmp_path: Path) -> None:
    ...
    for split in ("train", "validation"):
        policy = manifest["metrics"][split]["policy"]
        assert policy["by_input_request_type"]["SELECT_OPTION"]["count"] > 0
        assert policy["by_semantic_role"]["OPTION_SELECTION"]["count"] > 0
        assert policy["by_semantic_role"]["OPTION_SELECTION"]["pairwise_count"] > 0
        assert policy["by_can_skip"]["false"]["count"] > 0
    artifact_provenance = json.loads((paths["artifact_path"] / "provenance.json").read_bytes())
    assert artifact_provenance["metrics"] == manifest["metrics"]
