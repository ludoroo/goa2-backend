...
def test_hex_adjacency_is_exact_for_dynamic_geometry_sharing_a_map_id() -> None:
    state = _state()
    original = _observation(state)
    shifted = state.model_copy(deep=True)
    shifted.board.tiles = {
        Hex(q=hex_.q + 10, r=hex_.r, s=hex_.s - 10): tile.model_copy(
            update={"hex": Hex(q=hex_.q + 10, r=hex_.r, s=hex_.s - 10)}
        )
        for hex_, tile in shifted.board.tiles.items()
    }
    shifted_observation = _observation(shifted)

    def adjacency_pairs(observation: Any) -> list[tuple[str, str]]:
        return [
            (edge["source_ref"], edge["target_ref"]) for edge in _edges(observation, "HEX_ADJACENT")
        ]

    expected_shifted = [
        (source["local_ref"], target["local_ref"])
        for source in _tokens(shifted_observation, "TILE")
        for target in _tokens(shifted_observation, "TILE")
        if source != target
        and max(
            abs(source["features"][axis] - target["features"][axis]) for axis in ("q", "r", "s")
        )
        == 1
    ]

    assert adjacency_pairs(shifted_observation) == sorted(expected_shifted)
    assert len(adjacency_pairs(shifted_observation)) == len(adjacency_pairs(original))
