...
class HangingRunner:
    ...
    def __call__(self, case: GameCase) -> EvaluationGameResult:  # pragma: no cover - child
...
class ProgressHangingRunner(HangingRunner):
    """Hung runner that reports partial game progress before stalling."""
...
def _mix(a_wins: int, b_wins: int, max_steps: int = 0) -> list[EvaluationGameResult]:
...
# ``EvaluationGameResult(reason='wall_clock_timeout', winner_side=None, rounds=0,
...
def test_timed_protocol_rejects_non_picklable_runner_before_checkpoint(
    ...
    def _local_runner(case: GameCase) -> EvaluationGameResult:
...
def test_run_protocol_wall_clock_timeout_terminates_child_and_checkpoints_row(
    ...
    Behavior asserted (avoiding overspecification of the process supervisor):
      ...
      2. Every case yields an EvaluationGameResult with reason='wall_clock_timeout',
...
def test_timeout_row_preserves_latest_reported_progress(tmp_path: Path) -> None:
    results = run_protocol(
        _timeout_protocol(tmp_path),
        checkpoint_path=tmp_path / "obs.jsonl",
        run_case=ProgressHangingRunner(_hang_runner_marker_dir(tmp_path)),
    )

    assert {(obs.rounds, obs.steps) for obs in results} == {(7, 432)}
...
def _timeout_obs(case_id: str = "t", a_side: str = "RED") -> EvaluationGameResult:
    return EvaluationGameResult(
        case_id=case_id,
        world_seed=0,
        a_side=a_side,
