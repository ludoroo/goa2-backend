...
from automata.search import LEGACY_SCHEDULE_ID
...
@pytest.mark.parametrize(
    ...
    [
        (LeafMode.IMMEDIATE, 1),
        (LeafMode.IMMEDIATE_ACTION, 1),
        (LeafMode.STABLE_TURN, 1),
        (LeafMode.BOUNDED_CONTINUATION, 0),
        (LeafMode.BOUNDED_CONTINUATION, 1),
    ],
)
def test_repeated_forced_tree_decision_fails_closed(
    monkeypatch, leaf_mode: LeafMode, cutoff_limit: int
) -> None:
    from automata.search.ismcts import engine
    ...
    assert raised.value.root_search_plan.schedule_id == LEGACY_SCHEDULE_ID
