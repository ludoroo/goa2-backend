...
def test_loader_rejects_candidate_policy_tampering_and_exposes_canonical_digest(
    ...
    assert dataset.metadata.schema_version == 2
...
def test_current_loader_rejects_legacy_v1_rows_instead_of_reinterpreting_them(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(ValueError, match=r"schema_version|Input should be 2"):
        load_joint_dataset(path)
